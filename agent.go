...
func main() {
	...
	auth.POST("/api/ssh-keys", func(c *gin.Context) {
		...
		if _, stderr, err := runDockerExec(containerName, args, 20*time.Second); err != nil {
			log.Printf("[SSHKeys] ssh-keygen error: %v (stderr: %s)", err, strings.TrimSpace(stderr))
			c.JSON(http.StatusBadGateway, gin.H{"error": "ssh-keygen failed"})
			return
		}
		// Read keys
		priv, perr, err := runDockerExec(containerName, []string{"bash", "-lc", fmt.Sprintf("cat %s", base)}, 5*time.Second)
		if err != nil {
			...
			// attempt cleanup
			_, _, _ = runDockerExec(containerName, []string{"rm", "-f", base, base + ".pub"}, 5*time.Second)
			return
		}
		pub, puberr, err := runDockerExec(containerName, []string{"bash", "-lc", fmt.Sprintf("cat %s.pub", base)}, 5*time.Second)
		if err != nil {
	...
	r.POST("/webhook/github", func(c *gin.Context) {
		...
		if eventHeader == "issues" {
			if iss, ok := payload["issue"].(map[string]interface{}); ok {
				...
				issueTimeline := ""
				if tl, ok := iss["timeline_url"].(string); ok {
					issueTimeline = tl
				}
				issueUser := ""
				if u, ok := iss["user"].(map[string]interface{}); ok {
					if lg, ok := u["login"].(string); ok {
						issueUser = lg
					}
					if nm, ok := u["name"].(string); ok && issueUser == "" {
						issueUser = nm
					}
				}
				issueState := ""
				if st, ok := iss["state"].(string); ok {
					issueState = st
				}
				// Labels extraction
				...
				if rx, ok := iss["reactions"].(map[string]interface{}); ok {
					...
					case json.Number:
						if iv, err := strconv.Atoi(tv.String()); err == nil {
							reactionsTotal = iv
						}
					case string:
						if iv, err := strconv.Atoi(tv); err == nil {
							reactionsTotal = iv
						}
					}
				...
				html := ""
				if h, ok := iss["html_url"].(string); ok {
					html = h
				}
				if repoFull != "" && num != "" {
		...
		if eventHeader == "push" {
			if pushEvt, ok := parsedEvt.(*github.PushEvent); ok && pushEvt != nil {
				createdFlag := false
				if pushEvt.Created != nil {
					createdFlag = *pushEvt.Created
				}
				...
				for _, cm := range pushEvt.Commits {
					for _, f := range cm.Added {
						...
						if len(addedSamples) < 10 {
							addedSamples = append(addedSamples, f)
						}
					}
				}
				// Head commit details
				...
				if pushEvt.HeadCommit != nil {
					if pushEvt.HeadCommit.ID != nil {
						headID = *pushEvt.HeadCommit.ID
					}
					if pushEvt.HeadCommit.Message != nil {
						headMsg = *pushEvt.HeadCommit.Message
					}
		...
		if (eventHeader == "pull_request_review" || eventHeader == "pull_request_review_comment") && strings.EqualFold(sender, "gemini-code-assist[bot]") {
			...
			if pr, ok := payload["pull_request"].(map[string]interface{}); ok {
				...
				if h, ok := pr["head"].(map[string]interface{}); ok {
					if v, ok := h["ref"].(string); ok {
						headRef = v
					}
					if v, ok := h["sha"].(string); ok {
						headSHA = v
					}
				}
			} else {
			...
			if eventHeader == "pull_request_review" {
				...
				if cm, ok := payload["comment"].(map[string]interface{}); ok {
					...
					lineStr := ""
					if lv, ok := cm["line"].(float64); ok {
						lineStr = strconv.FormatInt(int64(lv), 10)
					}
	...
	r.POST("/webhook/jira", func(c *gin.Context) {
		...
		if len(comments) == 0 {
			if cm, ok := payload["comment"].(map[string]interface{}); ok {
				...
				if rb, ok := cm["renderedBody"].(string); ok && rb != "" {
					comments = append(comments, rb)
				}
			}
		}
