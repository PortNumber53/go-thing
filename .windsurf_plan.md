...
- Backend (`agent.go`):
  ...
  - New `GET /csrf` issues a random token, sets `csrf_token` cookie (SameSite=Strict, Secure when HTTPS), and returns `{ token }` in JSON.
...
### Login rate limiter cleanup optimization (2025-08-13)
- File: `agent.go` (`loginRateLimiter.cleanup`)
- Change: Replaced in-place deletions under a long-held mutex with a copy-and-swap approach. We snapshot maps under lock, rebuild filtered maps without holding the lock, then swap under lock.
- Reason: Minimizes mutex hold time during cleanup when maps are large, reducing contention with `/login` requests.
- Result: Better scalability and lower latency during periodic cleanup.
