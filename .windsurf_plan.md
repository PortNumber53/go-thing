...
- Backend (file: `agent.go`):
  ...
  - Added routes:
    - `GET /settings` → server-rendered HTML page with forms to update display name and change password.
...
- Usage:
  - Navigate to `/settings` when logged in. The page fetches `/me` and uses `/csrf` to include `X-CSRF-Token` on writes.
  - APIs accept/return JSON; ideal for future SPA integration as well.
- Build: `go build ./...` passes.
