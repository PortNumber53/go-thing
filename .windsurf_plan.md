...
### Centralized Email Validation (2025-09-07)
- Added `utility/validation.go` exposing `utility.EmailRegex()` and `utility.IsValidEmail()`.
- Updated `routes/signup_routes.go` to use `utility.IsValidEmail`.
- Removed duplicated email regex/cache from `agent.go` to avoid drift.

### Slack Home Error Logging (2025-09-07)
- Restored error logging for Slack App Home publishing in `routes/slack_routes.go`.
- Now logs `[Slack Home] publish failed for user <id>: <error>` when `utility.PublishSlackHomeTab(...)` returns an error.

### Migrations CLI Refactor (2025-09-07)
- Extracted unexported `runMigrateCLI` from `agent.go` into `utility/migrations.go` as exported `utility.RunMigrateCLI`.
- Updated `main()` in `agent.go` to call `utility.RunMigrateCLI(os.Args[2:])` when `os.Args[1] == "migrate"`.
- Removed the local function and the now-unused `flag` import from `agent.go`.
...
- AI follow-up loop (2025-08-20): In `agent.go` GitHub webhook handler, detect `pull_request_review` and `pull_request_review_comment` authored by `gemini-code-assist[bot]`, extract review/comment body and inline context, and asynchronously invoke `utility.GeminiAPIHandler` with task instructions to apply suggestions and respond `ALL_DONE!` when none remain. Logged results only for now.
  ...
  - Maintainability (2025-09-07): Replaced hardcoded occurrences of the bot username with a package-level constant `geminiBotUsername` in `routes/github_routes.go`.
