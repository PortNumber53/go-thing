...
- Implemented global rate limiting and backoff for Gemini API calls in `utility/gemini.go`.
  ...
  - Added best-effort retry loop for 429/RESOURCE_EXHAUSTED with:
    ...
    - Linear backoff fallback (5s * attempt), up to 3 attempts.
  ...
  - Maintainability (2025-08-21): Introduced `geminiModelName` constant in `utility/gemini.go` and used it in `callGeminiAPI()` to avoid hardcoded model strings and ease future upgrades/testing.
