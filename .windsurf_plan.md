...
- Implemented global rate limiting and backoff for Gemini API calls in `utility/gemini.go`.
  ...
  - Consistency (2025-08-22): Moved `maxBurst` from local const in `getGeminiLimiter()` to the package-level const block alongside `maxAttempts`, `baseDelay`, and `backoffJitterMs` for centralized configuration.
