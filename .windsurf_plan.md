...
1. Add new tools by creating a file in `tools/` with an `init()` that registers both metadata and executor.
2. Add basic unit tests for tool executors.
3. Validate `GET /api/tools` lists all registered tools and `POST` dispatch works end-to-end.
4. Add an integration test for `agent.go` to ensure it correctly reflects the tool server's registry in prompts and delegates execution.
5. Document required config for `jira_search_issues` (`JIRA_URL`, `JIRA_TOKEN`, optional `JIRA_EMAIL`).
