...
- Implemented global rate limiting and backoff for Gemini API calls in `utility/gemini.go`.
  ...
  - Reliability (2025-08-21): Seeded `math/rand` in `agent.go` `init()` with `time.Now().UnixNano()` so backoff jitter is non-deterministic across instances, avoiding synchronized retries.
